...
public class NumericValidator<T> : Validator<T>, INumericValidator where T : struct, INumber<T> {
	...
	protected T2 IncrementWithCheck<T2>(T2 value, T2 amount) where T2 : INumber<T2>, IMinMaxValue<T2> {
		try {
			checked {
				return value + amount;
			}
	...
	protected T2 IncrementWithCheck<T2>(T2 value, T2 amount, int? _ = default) where T2 : INumber<T2> => value + amount;
	...
	protected T2 DecrementWithCheck<T2>(T2 value, T2 amount) where T2 : INumber<T2>, IMinMaxValue<T2> {
		try {
			checked {
				return value - amount;
			}
	...
	/// </summary>
	/// <returns>(<paramref name="value"/> - <paramref name="amount"/>)</returns>
	protected T2 DecrementWithCheck<T2>(T2 value, T2 amount, int? _ = default) where T2 : INumber<T2> => value - amount;
	...
	/// </summary>
	protected T2 ValidateWithTypeCheck<T2>(T2 value, T2 stepStart, T2 stepAmount)
	...
	protected T2 ValidateWithTypeCheck<T2>(T2 value, T2 stepStart, T2 stepAmount, int? _ = default)
	...
	protected T2 ValidateWithTypeCheck<T2>(T2 value, T2 stepStart, T2 stepAmount, bool? _ = default) =>
